	...
	<div class="dock">
		<div class="dock__left">
			<xDropdown trigger="click" placement="top-start">
				<button class="dock__launcher" type="button" title="打开应用台">
				...
				<xDropdownMenu slot="dropdown">
					<div class="dock__launcher-menu">
						<xBtn
							preset="text"
							class="dock__launcher-menu-btn"
							@click="openAppById('api')">
							<span class="dock__launcher-menu-btn-content">
								<span
									class="dock__launcher-menu-icon"
									:style="{ color: getAppColor('api') }">
						...
						<xBtn
							preset="text"
							class="dock__launcher-menu-btn"
							@click="openAppById('explore')">
							<span class="dock__launcher-menu-btn-content">
								<span
									class="dock__launcher-menu-icon"
									:style="{ color: getAppColor('explore') }">
					...
					<div class="dock__preview">
							...
							<div class="dock__preview-title">
								...
								<small>{{
									getAppWindows(app.id).length
										? getAppWindows(app.id).length + " 个窗口"
										: "已固定应用"
								}}</small>
							</div>
							<button
						...
						<div v-if="getAppWindows(app.id).length" class="dock__preview-list">
							<div
								...
								:class="{
									'dock__preview-item--active':
										_.$ModalManager.getFocusedId() === win.id &&
										!win.isMinimized
								}"
								role="button"
								tabindex="0"
						...
						<xTooltip effect="dark" placement="top">
							...
							<button
								...
								<span
									...
									:class="{
										'dock__indicator--active': isAppActive(app.id)
									}"></span>
								<span
		...
		<div class="dock__right">
			...
			<xTooltip effect="dark" placement="top">
				...
				<button class="dock__settings" title="打开设置" @click="handleSettingsClick">
	...
	return {
		...
		computed: {
			...
			dockApps() {
				...
				const pinnedApps =
					this.system && this.system.pinnedApps ? this.system.pinnedApps : [];
				const openWindows =
					this.system && _.$ModalManager.getAllInstances()
						? _.$ModalManager.getAllInstances()
						: [];
				const seen = {};
				const orderedIds = [];
		...
		methods: {
			...
			isAppActive(appId) {
				...
				const activeWindow = this.getAppWindows(appId).find(
...
.dock {
	...
	&__launcher-orb {
		...
		background: linear-gradient(
			135deg,
			var(--v1-shell-primary, var(--el-color-primary)) 0%,
			var(--el-color-primary-light-5, #93c5fd) 100%
		);
		filter: blur(1px);
		opacity: 0.78;
	}
