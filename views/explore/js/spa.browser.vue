	...
	window.spa.browser = (function () {
		...
		var toggleSortField = function (field) {
			...
			if (idx === -1) {
				...
				stateMap.sortConfig[idx].order =
					stateMap.sortConfig[idx].order === "asc" ? "desc" : "asc";
		...
		var getIcon = function (type, file) {
			...
			if (file && (type === "image" || type === "video")) {
				...
				return (
					'<img class="file-item__preview" src="' +
					previewUrl +
					'" loading="lazy" onerror="$(this).hide().next().show();">' +
		...
		var render = async function (targetMode) {
			...
			stateMap.$container
				.removeClass("file-browser--list file-browser--grid")
				.addClass("file-browser--" + mode);
