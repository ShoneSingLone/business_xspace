	...
	(function () {
		...
		var renderPreview = function (file) {
			...
			switch (file.type) {
				...
				case "video":
					var isRmvb =
						file.name.toLowerCase().endsWith(".rmvb") ||
						file.name.toLowerCase().endsWith(".rm") ||
						file.rawType === "rmvb";
					if (isRmvb) {
						$content.append(
							[
					...
					var $video = $(
						'<video class="viewer__media max-w-[90%] max-h-[80%] object-contain shadow-2xl" playsinline></video>'
					);
		...
		var initModule = function ($container) {
			...
			$("#spa-viewer-download").on("click", function () {
				if (
					stateMap.currentFile &&
					(stateMap.currentFile.downloadUrl || stateMap.currentFile.url)
				) {
